 ...
 * #L%
 */
import de.hub.cses.ces.entity.market.Supply;
 ...
 */
@Entity
@Table(name = "economy_dynamic_supply_tbl")
@NamedQueries({
    @NamedQuery(name = "DynamicSupply.findByProduct", query = "SELECT s FROM DynamicSupply s WHERE s.product = :product ORDER BY s.calculated DESC")
})
public class DynamicSupply extends Supply {
     ...
     * @param calculated
     */
    public DynamicSupply(Long id, Product product, int quantity, int availableQuantity, double pricePerUnit, Date calculated) {
